...
| 타임아웃 | 15분 |
| 단계 | `npm ci` → `npm run lint` → `compile` → `test` → `spotbugs:check` → `pmd:check` → `package -DskipTests` |
| 산출물 | `target/*.jar` (아티팩트 보관 1일) |
| 리포트 | Surefire XML → PR Check + 코멘트 / SpotBugs·PMD·ESLint·HTMLHint 위반 시 빌드 실패로 Check 차단 + XML 아티팩트 업로드 |
