...
**화면 목적**: 오류 처리 핸들러 APP을 등록/관리하는 화면이다. 오류 발생 시 자동으로 실행되는 핸들러의 기본 정보와 구현 클래스를 관리한다.

**접근 경로**: GNB > 오류 관리 > 오류 핸들러 관리
...
| 핸들러 ID | 텍스트 | 가능 | - |
| 핸들러명 | 텍스트 | 가능 | - |
| 구현 클래스명 | 텍스트 | 가능 | 패키지 포함 전체 클래스 경로 |
| 설명 | 텍스트 | 가능 | - |
| 사용 여부 | 상태 아이콘 | 불가 | Y/N, 초록/빨강 원형 아이콘 |

- 목록 클릭 시 동작: 오류 핸들러 수정 팝업 열림
...
| 구현 클래스명 | 텍스트 입력 | 필수 | 가능 | 최대 200자, 패키지 포함 전체 클래스 경로 |
| 설명 | 텍스트 영역 | 선택 | 가능 | 최대 200자 |
...
| 버튼명 | 노출 조건 | 클릭 동작 | 권한 제어 |
|--------|-----------|-----------|-----------|
| 등록 | 항상 노출 | 오류 핸들러 등록 팝업 열기 | 쓰기 권한 보유자만 |
| 엑셀 | 항상 노출 | 검색 결과 엑셀 다운로드 | 전체 |
| 출력 | 항상 노출 | 목록 인쇄 | 전체 |
| 저장 (팝업) | 팝업 열림 시 | 오류 핸들러 정보 저장 | 쓰기 권한 |
| 삭제 (팝업) | 수정 모드 시 | 확인 팝업 후 오류 핸들러 삭제 | 쓰기 권한 |

### 개별 기능 요구사항

- 핸들러 ID는 등록 후 변경이 불가하여야 한다.
- 구현 클래스명은 Java 패키지 경로를 포함한 전체 클래스명(FQCN)을 입력하여야 한다.
- 오류코드에 할당된 핸들러를 삭제하려는 경우, "해당 핸들러가 오류코드에 할당되어 있습니다. 삭제하시겠습니까?" 경고 확인 팝업이 표시되어야 한다.
- 사용 여부를 N으로 변경 시 해당 핸들러가 할당된 오류코드가 있으면 경고 메시지가 표시되어야 한다.

### 비기능 요구사항

| 항목 | 요구사항 |
|------|----------|
| 응답 속도 | 오류 핸들러 목록 조회 결과가 3초 이내에 표시되어야 한다 |

---

## 오류 핸들러 수행 내역

**화면 목적**: 오류 핸들러의 실행 이력과 결과를 조회하는 화면이다 (조회 전용). 핸들러가 언제 어떤 오류에 대해 실행되었고, 그 결과가 성공인지 실패인지를 추적할 수 있다.

**접근 경로**: GNB > 오류 관리 > 오류 핸들러 수행 내역
...
| 영역 | 설명 |
|------|------|
| 검색 필터 영역 | 상단, 핸들러 ID + 오류 코드 + 수행일 날짜범위 + 수행 결과 드롭다운 + 조회 건수 + 조회 버튼 |
| 도구 모음 | 목록 상단 우측, 엑셀 / 출력 버튼 |
| 목록 영역 | 중앙, 오류 핸들러 수행 내역 테이블 |
| 페이지 네비게이션 | 목록 하단 |
...
| 필터 항목명 | 입력 유형 | 필수 여부 | 기본값 | 유효성 조건 |
|-------------|-----------|-----------|--------|-------------|
| 핸들러 ID | 텍스트 입력 | 선택 | 빈 값 | - |
| 오류 코드 | 텍스트 입력 | 선택 | 빈 값 | - |
| 수행일 (시작) | 날짜 선택 | 선택 | 1개월 전 | - |
| 수행일 (종료) | 날짜 선택 | 선택 | 오늘 | 시작일 이후여야 함 |
| 수행 결과 | 드롭다운 (전체 / 성공 / 실패) | 선택 | 전체 | - |
...
| 핸들러명 | 텍스트 | 가능 | - |
| 오류 코드 | 텍스트 | 가능 | - |
| 수행 일시 | 날짜/시간 | 가능 | YYYY-MM-DD HH:mm:ss |
| 수행 결과 | 상태 배지 | 불가 | 성공=초록, 실패=빨강 |
| 처리 시간(ms) | 숫자 | 가능 | 밀리초 단위 |
| 오류 메시지 | 텍스트 | 불가 | 말줄임 처리, 실패 시에만 표시 |

- 목록 클릭 시 동작: 수행 내역 상세 팝업 열림
- 기본 정렬 기준: 수행 일시 내림차순
- 페이지당 기본 표시 건수: 20건
...
| 오류 코드 | 텍스트 | - |
| 오류 제목 | 텍스트 | - |
| 수행 일시 | 날짜/시간 | YYYY-MM-DD HH:mm:ss |
| 수행 결과 | 상태 배지 | 성공=초록, 실패=빨강 |
| 처리 시간(ms) | 숫자 | 밀리초 단위 |
| 수행 파라미터 | 텍스트 영역 (읽기 전용) | JSON 또는 key=value 형식 |
| 수행 결과 상세 | 텍스트 영역 (읽기 전용) | 핸들러 반환 결과 |
| 오류 스택트레이스 | 텍스트 영역 (읽기 전용) | 실패 시에만 표시, 코드 블록 스타일 |
...
- 수행일 검색 범위의 기본값은 최근 1개월이어야 한다.
- 목록에서 행 클릭 시 해당 수행 내역의 상세 정보 팝업이 열려야 한다.
- 상세 팝업에서 오류 스택트레이스는 실패 건에 대해서만 표시되어야 하며, 코드 블록(monospace) 스타일로 가독성을 높여야 한다.
- 수행 파라미터 및 수행 결과 상세 영역은 내용이 많을 경우 스크롤이 가능하여야 한다.
- 수행 결과 배지는 색상으로 시각 구분되어야 한다 (성공=초록, 실패=빨강).

### 비기능 요구사항

| 항목 | 요구사항 |
|------|----------|
| 응답 속도 | 수행 내역 목록 조회 결과가 3초 이내에 표시되어야 한다 |
