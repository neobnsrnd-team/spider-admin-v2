...
**화면 목적**: 시스템 전체에서 공통으로 사용하는 필드 ID와 국문/영문 기본값을 맵핑하여 관리하는 화면이다.

**접근 경로**: GNB > 거래/전문 관리 > 공통필드매핑 관리
...
| 영역 | 설명 |
|------|------|
| 검색 필터 영역 | 상단, 공통필드 ID + 국문 기본값 + 영문 기본값 + 조회 건수 + 조회 버튼 |
| 도구 모음 | 목록 상단 우측, 엑셀 / 출력 버튼 |
| 목록 영역 | 중앙, 공통필드 맵핑 목록 테이블 |
| 페이지 네비게이션 | 목록 하단 |
| 버튼 영역 | 목록 하단, 등록 버튼 |
...
| 필터 항목명 | 입력 유형 | 필수 여부 | 기본값 | 유효성 조건 |
|-------------|-----------|-----------|--------|-------------|
| 공통필드 ID | 텍스트 입력 | 선택 | 빈 값 | - |
| 국문 기본값 | 텍스트 입력 | 선택 | 빈 값 | - |
| 영문 기본값 | 텍스트 입력 | 선택 | 빈 값 | - |
...
- 목록 클릭 시 동작: 공통필드 맵핑 수정 팝업 열림
- 기본 정렬 기준: 공통필드 ID 오름차순
- 페이지당 기본 표시 건수: 20건
...
| 항목명 | 입력 유형 | 필수 여부 | 수정 가능 여부 | 유효성 조건 |
|--------|-----------|-----------|----------------|-------------|
| 공통필드 ID | 텍스트 입력 | 필수 | 등록 시만 가능 | 영문/숫자/언더스코어, 최대 100자 |
| 국문 기본값 | 텍스트 입력 | 필수 | 가능 | 최대 200자 |
| 영문 기본값 | 텍스트 입력 | 선택 | 가능 | 최대 200자 |
| 설명 | 텍스트 영역 | 선택 | 가능 | 최대 200자 |
...
| 버튼명 | 노출 조건 | 클릭 동작 | 권한 제어 |
|--------|-----------|-----------|-----------|
| 등록 | 항상 노출 | 공통필드 맵핑 등록 팝업 열기 | 쓰기 권한 |
| 엑셀 | 항상 노출 | 검색 결과 엑셀 다운로드 | 전체 |
| 출력 | 항상 노출 | 목록 인쇄 | 전체 |
| 저장 (팝업) | 팝업 열림 시 | 맵핑 정보 저장 | 쓰기 권한 |
| 삭제 (팝업) | 수정 모드 시 | 확인 팝업 후 삭제 | 쓰기 권한 |
