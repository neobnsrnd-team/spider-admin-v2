...
- 백업/복원 기능:
  - 백업 단위: 전문 단위 (전문 구조 + 필드 정의 전체)
  - 백업 생성: 수정 모드에서 "백업" 버튼 클릭 시 현재 상태가 저장되어야 한다. 백업 시 메모(사유) 입력을 선택적으로 할 수 있어야 한다.
  - 백업 목록 조회: 백업 이력(일시, 수행자, 메모)이 목록으로 표시되어야 한다.
  - 복원: 백업 목록에서 항목을 선택하고 "복원" 버튼을 클릭하면 해당 시점으로 전체 복원되어야 한다. 복원 전 "현재 데이터가 백업 시점({YYYY-MM-DD HH:mm})의 데이터로 대체됩니다. 계속하시겠습니까?" 확인 팝업이 표시되어야 한다.
